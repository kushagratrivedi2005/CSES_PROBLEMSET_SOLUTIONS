...
void dfs(int src,int par){
    for(auto child:adj[src]){
        ...
        dfs(child,src);
        if(colors[src].size()<colors[child].size()){
            swap(colors[child],colors[src]);
        }
        for(auto i:colors[child]){
            colors[src].insert(i);
        }
    }
        dist[src]=colors[src].size();
 
}
void solve(int xyz)
{
    int n;cin>>n;
    REP(i,1,n+1){int x;cin>>x;colors[i].insert(x);}
    REP(i,0,n-1){
        int y,z;cin>>y>>z;
        adj[y].pb(z);
        adj[z].pb(y);
