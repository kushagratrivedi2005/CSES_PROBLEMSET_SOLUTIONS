...
*/
  int n,k1,k2;
  int get(deque<int>&arr,int index){
    if(index<0)return arr[0];
    if(index>=0 && index<arr.size())return arr[index];
    return 0;
  }
  long long  ans=0;
  void mergearrays(deque<int>&arra,deque<int>&arrb){
    if(arra.size()<arrb.size()){swap(arra,arrb);}
    for(int i=0;i<arrb.size();i++){
        ans+=1ll*(get(arrb,i)-get(arrb,i+1))*(get(arra,k1-i)-get(arra,k2-i+1)); 
    }
    for(int i=0;i<arrb.size();i++){
        arra[i]+=arrb[i];
    }
  }
  deque<int>suffix(int src,int par){
    deque<int>parentarr={1};
    for(int child:adj[src]){
        if(par==child)continue;
        deque<int>childarr=suffix(child,src);
        childarr.push_front({childarr.front()});
        mergearrays(parentarr,childarr);
    }
    return parentarr;
  }
