...
{
  int n,m;cin>>n>>m;
  bitset<50010>values[n+1];
  vector<vector<int>>graph(n+1);
  vector<int>indegree(n+1);
  REP(i,0,m){
    int a,b;cin>>a>>b;graph[b].pb(a);indegree[a]++;
  }
  REP(i,1,n+1){values[i].set(i);}
  queue<int>q;
  REP(i,1,n+1){
    if(indegree[i]==0){
        q.push(i);
  ...
  while(!q.empty()){
    ...
    for(auto i:graph[curr]){
        ...
        indegree[i]--;if(indegree[i]==0)q.push(i);
    }
  }
  REP(i,1,n+1){cout<<values[i].count()<<" ";}
}
