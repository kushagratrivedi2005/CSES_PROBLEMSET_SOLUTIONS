...
▀▀█▀▀ █▀▀█ ▀█▀  █   █  █▀▀▀ █▀▀▄ ▀█▀ 
  ...
  █   █  █ ▄█▄   ▀▄▀   █▄▄▄ █▄▄▀ ▄█▄ 
*/
int n;int parent[100001];int rank1[100001];stack<ii>roller;
int curr=-1;int components;int compsize[100001];
void init(int n){
    components=n;
    REP(i,0,100001){rank1[i]=1;parent[i]=i;}
}
int find(int x){
...
void rollback(int state){
    while(curr>state){
        ...
        rank1[now.first]-=rank1[now.second];
        components=compsize[curr];
        curr--;roller.pop();
    }
}
vii answers;
...
void dfs(int index,int l,int r){
    ...
    if(l==r){
        if(query[r]!=-1){
            answers.pb({query[l],components});
        }}
        else{
            int mid=l+(r-l)/2;
            dfs(2*index+1,l,mid);
            dfs(2*index+2,mid+1,r);
        }
        rollback(prev);
}
void solve(int xyz)
{
    ...
    REP(i,0,m){
        int x,y;cin>>x>>y;if(x>y)swap(x,y);range[{x,y}]=0;
    }
    query[0]=0;
    REP(i,1,k+1){
        int x;cin>>x;
        if(x==1){
            int x,y;cin>>x>>y;if(x>y)swap(x,y);range[{x,y}]=i;
        }
        else{
            int x,y;cin>>x>>y;if(x>y)swap(x,y);
            insertr={x,y};
            update(0,0,k,range[insertr],i-1);
                {range.erase(range.find(insertr));}
        }
        query[i]=i;
    }
    for(auto i:range){
        insertr=i.first;
        update(0,0,k,i.second,k);
    }
    dfs(0,0,k);
