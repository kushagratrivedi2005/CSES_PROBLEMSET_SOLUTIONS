...
int size1[200005];
void reset(){
    REP(i,0,200005){par[i]=i;size1[i]=1;}
}
int find(int x){
    if(x==par[x])return x;
...
void join(int x,int y){
    x=find(x);y=find(y);
    if(x!=y){if(size1[x]<size1[y])swap(x,y);}
    par[y]=x;
    size1[x]+=size1[y];
}
...
void solve(int xyz)
{
 ...
 vector<int>ans(q,-1);
 REP(i,0,q){range[i]={0,m-1};}
 while(1){
    reset();
    vii mid;
    REP(i,0,q){if(range[i].first<=range[i].second){
        int m=(range[i].first+range[i].second)/2;mid.push_back({m,i});
    }}
    if(mid.empty())break;
    ...
    REP(i,0,m){
        ...
        while(j<mid.size()&& i==mid[j].first){
            int k=mid[j].second;
            if(queries[k].first==queries[k].second){
                ans[k]=0;range[k]={1000,1};
            }
            else if(find(queries[k].first)==find(queries[k].second)){
                ans[k]=i+1;range[k]={range[k].first,i-1};
            }
            else{
