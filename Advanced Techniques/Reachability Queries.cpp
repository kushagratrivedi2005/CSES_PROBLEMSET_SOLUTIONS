...
void process(int i,vector<vector<int>>&rgraph){
    for(auto j:rgraph[i])supgraph[i]|=supgraph[j];
}
void solve(int xyz)
{
    int n,m,q;cin>>n>>m>>q;
    ...
    REP(i,0,n){
        ...
        // cout<<i<<" "<<j<<endl;
        // cout<<groups[i]<<" "<<groups[j]<<endl;
        rgraph[groups[j]].pb(groups[i]);}
    }
    
    for(int i=condensed.size()-1;i>=0;i--){
        process(condensed[i],rgraph);
    }
    while(q--){
        int x,y;cin>>x>>y;x--;y--;
        if(supgraph[groups[x]].test(groups[y])){
            cout<<"YES"<<endl;
        }
        else{
            cout<<"NO"<<endl;
