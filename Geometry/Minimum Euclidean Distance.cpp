...
void solve(int xyz)
{
  int n;cin>>n;vii values(n);REP(i,0,n)cin>>values[i].first>>values[i].second;
  sort(values.begin(),values.end());
  set<ii>inrange;
  int j=0;
  int min_dist=8e18;
  REP(i,0,n){
    int d=ceil(sqrt(min_dist));
    while(values[i].first-values[j].first>=min_dist){
        inrange.erase({values[j].second,values[j].first});
        j++;
    }
    auto lower=inrange.lower_bound({values[i].second-d,3820320});
    auto higher=inrange.lower_bound({values[i].second+d,322332});
    for(auto it=lower;it!=higher;it++){
        int dx=values[i].first-it->second;
        int dy=values[i].second-it->first;
        min_dist=min(min_dist,1ll*dx*dx+1ll*dy*dy);
    }
    inrange.insert({values[i].second,values[i].first});
  }
  cout<<min_dist;
}
