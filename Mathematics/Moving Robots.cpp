...
double numdirections(int x,int y){
    int ans=0;
    REP(i,0,4){
        if(isvalid(x+moves[i].first,y+moves[i].second))ans++;
    }
    return ans;
}
...
void solve(int xyz)
{
  ...
  REP(k,1,n+1){
    REP(i,0,8){
        REP(j,0,8){
            REP(l,0,4){
                int x1=i+moves[l].first;int y1=j+moves[l].second;
                if(isvalid(x1,y1)){dp[k][i][j]+=dp[k-1][x1][y1]/numdirections(x1,y1);}
            }
        }
    }
  }
...
//   }
  ...
  cout<<fixed<<setprecision(6)<<ans1<<endl;return;
}
