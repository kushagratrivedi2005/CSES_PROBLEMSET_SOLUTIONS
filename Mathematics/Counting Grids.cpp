...
void solve(int xyz)
{
    ...
    if(n&1){
        int ans=0;
        ans=addmod(ans,power(2,(n*n-1)/4));
        ans=addmod(ans,power(2,(n*n-1)/4));
        ans=addmod(ans,power(2,(n*n-1)/2));
        ans=multmod(ans,2);
        ans=addmod(ans,power(2,n*n));
        ans=multmod(ans,inv[4]);
        cout<<ans<<endl;return;
    }
    else{
int ans=0;
        ans=addmod(ans,power(2,(n*n)/4));
        ans=addmod(ans,power(2,(n*n)/4));
        ans=addmod(ans,power(2,(n*n)/2));
        ans=addmod(ans,power(2,n*n));
        ans=multmod(ans,inv[4]);
        cout<<ans<<endl;return;
    }
