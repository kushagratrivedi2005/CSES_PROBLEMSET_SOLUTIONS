...
void solve(int xyz)
{
    ...
    int total=0;
    n/=2;
    REP(i,0,s.length()){if(s[i]=='(')k++;else{k--;total++;}if(k<0){cout<<0<<endl;return;}}
    n-=k;n-=total;
    REP(i,0,n+1){dp[i]=1;}
    REP(i,1,n+1){
        dp[i]=multmod(dp[i-1],dp[i]);
        dp[i]=multmod(dp[i],2*i+k-1);
        dp[i]=multmod(dp[i],2*i+k);
        dp[i]=multmod(dp[i],inv[i]);
        dp[i]=multmod(dp[i],inv[i+k+1]);
    }
