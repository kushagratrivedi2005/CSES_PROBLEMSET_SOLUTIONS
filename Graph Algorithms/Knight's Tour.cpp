...
bool isvalid(vector<vector<int>>&chess,int x,int y){
    return x>=0 && y>=0 && x<8 && y<8 && chess[x][y]==0;
}
 
int availablemoves(int x,int y,vector<vector<int>>&chess){
    int ans=0;
    REP(i,0,8){
        int x1=x+moves[i].first; int y1=y+moves[i].second;
        if(isvalid(chess,x1,y1))ans++;
    }
    return ans;
}
bool construct(vector<vector<int>>&chess,int x,int y,int number){
    chess[x][y]=number;
    if(number==64){
        print(chess);
        return true;
    ...
    REP(i,0,8){
        int x1=x+moves[i].first; int y1=y+moves[i].second;
        if(!isvalid(chess,x1,y1))continue;
        int z=availablemoves(x1,y1,chess);
        available.pb({z,i});
    }
    sort(available.begin(),available.end());
    REP(i,0,available.size()){
    int x1=x+moves[available[i].second].first;
    int y1=y+moves[available[i].second].second;
    if(construct(chess,x1,y1,number+1))
    return true;
    }
    chess[x][y]=0;
    return false;
