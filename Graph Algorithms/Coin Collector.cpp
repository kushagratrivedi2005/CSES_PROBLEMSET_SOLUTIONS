...
▀▀█▀▀ █▀▀█ ▀█▀  █   █  █▀▀▀ █▀▀▄ ▀█▀ 
  ...
  █   █  █ ▄█▄   ▀▄▀   █▄▄▄ █▄▄▀ ▄█▄ 
*/
void dfs(vector<int>&visited,vector<vector<int>>&graph,stack<int>&st,int n){
    visited[n]=1;
    for(auto i:graph[n]){
        if(!visited[i]){dfs(visited,graph,st,i);}
    }
    st.push(n);
}
void dfs2(vector<int>&visited,vector<vector<int>>&graph,int n,int x,vector<int>&groups){
    visited[n]=1;
    groups[n]=x;
    for(auto i:graph[n]){
        if(!visited[i]){dfs2(visited,graph,i,x,groups);}
    }
}
int dp[200001];
int process(int i,vector<vector<int>>&rgraph,vector<int>&supercoins){
    if(dp[i])return dp[i];
    dp[i]=supercoins[i];
    for(auto j:rgraph[i])dp[i]=max(dp[i],dp[j]+supercoins[i]);
    return dp[i];
}
void solve(int xyz)
{
    ...
    int n,m;cin>>n>>m;
    vector<int>coins(n);REP(i,0,n)cin>>coins[i];
    vector<vector<int>>graph(n);
     vector<vector<int>>reverse(n);
    REP(i,0,m){
        int x,y;cin>>x>>y;x--;y--;graph[x].pb(y);reverse[y].pb(x);
    }
     vector<int>visited(n,0);stack<int>st;
     REP(i,0,n){
        if(!visited[i]){dfs(visited,graph,st,i);}
     }
     vector<int>groups(n);
     ...
     vector<int>supercoins(n);
 
     while(!st.empty()){
        int x=st.top();st.pop();
        if(!visited2[x]){
        dfs2(visited2,reverse,x,x,groups);
        condensed.pb(x);
        }
     }
    REP(i,0,n){supercoins[groups[i]]+=coins[i];}
    vector<vector<int>>rgraph(n);
    REP(i,0,n){
        for(auto j:reverse[i]){
        if(groups[i]==groups[j])continue;
        rgraph[groups[i]].pb(groups[j]);}
    }
    int ans=0;
    for(auto i:condensed){
        process(i,rgraph,supercoins);
        ans=max(ans,dp[i]);
    }
    cout<<ans<<endl;
}
