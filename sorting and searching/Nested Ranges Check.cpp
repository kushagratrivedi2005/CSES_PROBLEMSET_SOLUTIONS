...
▀▀█▀▀ █▀▀█ ▀█▀  █   █  █▀▀▀ █▀▀▄ ▀█▀ 
  ...
  █   █  █ ▄█▄   ▀▄▀   █▄▄▄ █▄▄▀ ▄█▄ 
*/
int comp(array<int,3> a,array<int,3> b){
    if(a[0]==b[0]){return a[1]<b[1];}
    return a[0]>b[0];
}
int comp2(array<int,3> a,array<int,3> b){
    if(a[0]==b[0]){return a[1]>b[1];}
    return a[0]<b[0];
}
void solve(int xyz)
{
    int n;cin>>n;vector<array<int,3>> values(n);REP(i,0,n){cin>>values[i][0]>>values[i][1];values[i][2]=i;}
    vector<int>ans1(n); vector<int>ans2(n);
    sort(values.begin(),values.end(),comp);
    int mini=INT_MAX;
    REP(i,0,n){
        if(values[i][1]>=mini)ans1[values[i][2]]=1;else{ans1[values[i][2]]=0;}
        mini=min(mini,values[i][1]);
    }
    sort(values.begin(),values.end(),comp2);
    int maxi=INT_MIN;
    REP(i,0,n){
  
        if(values[i][1]<=maxi)ans2[values[i][2]]=1;else{ans2[values[i][2]]=0;}
