...
void solve(int xyz)
{
  ...
  int ans=1;
  while(1){
    if(freq[values[j+1]]>0 || distinct<q ){
        j++;ans+=(j-i+1);if(freq[values[j]]==0)distinct++;freq[values[j]]++;
        // cout<<i<<" "<<j<<" "<<distinct<<" "<<ans<<endl;
        if(j==n-1)break;
    }
    else{
    while(1){
        freq[values[i]]--;if(freq[values[i]]==0){distinct--;i++;break;}i++;
    }}
  }
  cout<<ans;
}
