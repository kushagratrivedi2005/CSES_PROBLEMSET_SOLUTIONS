...
void solve(int xyz)
{
    int n;cin>>n;vector<array<int,3>> values(n);REP(i,0,n){cin>>values[i][0]>>values[i][1];values[i][2]=i;b.insert(values[i][1]);}
    vector<int>ans1(n,0); vector<int>ans2(n,0);
        int cnt = 0;
        map<int,int> mp;
    for (auto i: b) {
        mp[i] = ++cnt;
    ...
    REP(i,0,n){
        int y=query(mp[values[i][1]]);
        ans1[values[i][2]]=y;
        update(200005,mp[values[i][1]],1);
    }
    memset(segtree,0,sizeof(segtree));
    sort(values.begin(),values.end(),comp2);
    REP(i,0,n){
         int y=query2(mp[values[i][1]]);
        ans2[values[i][2]]=i-y;
        update(200005,mp[values[i][1]],1);
