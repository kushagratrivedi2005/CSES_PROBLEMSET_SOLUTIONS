...
void solve(int xyz)
{
  int n,k;cin>>n>>k;vii values(n);REP(i,0,n){cin>>values[i].first>>values[i].second;}
    sort(values.begin(),values.end(),comp);
    int ans=0;multiset<int>ends;
    REP(i,0,k)ends.insert(0);
    REP(i,0,n){
        auto z=ends.upper_bound(values[i].first);
        if(z==ends.begin())continue;
        {ends.erase(--(z));ans++;ends.insert(values[i].second);}
    }
    cout<<ans<<endl;
}
