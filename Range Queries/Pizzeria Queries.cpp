...
*/
int seg[800005];
void createseg(int index,int low,int high,vector<int>&values){
    if(low==high){
    seg[index]=values[low]-low;
    return;}
    int mid=(low+high)/2;
    createseg(2*index+1,low,mid,values);
    createseg(2*index+2,mid+1,high,values);
...
int seg2[800005];
void createseg2(int index,int low,int high,vector<int>&values){
    if(low==high){
    seg2[index]=values[low]+low;
    return;}
    int mid=(low+high)/2;
    createseg2(2*index+1,low,mid,values);
    createseg2(2*index+2,mid+1,high,values);
    seg2[index]=min(seg2[2*index+1],seg2[2*index+2]);
}
 
void updateseg2(int index,int low,int high, int i, int val){
    if(low==high){
    seg2[index]=val;
    return;}
    int mid=(low+high)>>1;
    if(i<=mid) updateseg2(2*index+1,low,mid,i,val);
    else updateseg2(2*index+2,mid+1,high,i,val);
    seg2[index]= min(seg2[2*index+1],seg2[2*index+2]);
 
}
 
int query2(int index, int low, int high, int l, int r) {
    if (high < l || low > r)
        return INT_MAX;
    if (low >= l && high <= r)
        return seg2[index];
    int mid = (high + low) / 2;
    int q1 = query2(2 * index + 1, low, mid, l, r);
    int q2 = query2(2 * index + 2, mid + 1, high, l, r);
    return min(q1, q2);
}
void solve(int xyz)
{
  int n,q;cin>>n>>q;vector<int>values(n);REP(i,0,n)cin>>values[i];
    createseg(0,0,n-1,values);
    createseg2(0,0,n-1,values);
    while(q--){
        int x;cin>>x;
        if(x==2){
            int z;cin>>z;
            cout<<min(z-1+query(0,0,n-1,0,z-1),query2(0,0,n-1,z-1,n-1)-z+1)<<endl;
        }
        else{
            int z,j;cin>>z>>j;
            updateseg(0,0,n-1,z-1,j-z+1);
            updateseg2(0,0,n-1,z-1,j+z-1);
        }
    }
}
