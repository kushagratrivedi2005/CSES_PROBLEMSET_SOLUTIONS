...
int query(int ind,int low,int high,int val){
    ...
    else{return query(2*ind+2,mid+1,high,val);}
}
 
void update(int ind,int low,int high,int x,int val){
    if(low==high){segtree[ind]-=val;return;}
    int mid=(low+high)/2;
    if(x<=mid){update(2*ind+1,low,mid,x,val);}
    else{update(2*ind+2,mid+1,high,x,val);}
    segtree[ind]=max(segtree[2*ind+1],segtree[2*ind+2]);
}
 
void solve(int xyz)
{
  int n,m;cin>>n>>m;vector<int>values(n);REP(i,0,n)cin>>values[i];
  build(0,0,n-1,values);
  while(m--){
    int x;cin>>x;
    if(x>segtree[0]){cout<<0<<" ";continue;}
    int y=query(0,0,n-1,x);
    update(0,0,n-1,y,x);
    cout<<y+1<<" ";
  }
}
