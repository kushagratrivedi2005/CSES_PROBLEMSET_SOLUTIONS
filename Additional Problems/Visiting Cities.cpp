...
▀▀█▀▀ █▀▀█ ▀█▀  █   █  █▀▀▀ █▀▀▄ ▀█▀ 
  ...
  █   █  █ ▄█▄   ▀▄▀   █▄▄▄ █▄▄▀ ▄█▄ 
*/
// code for constructing dominator tree for dijkstra tree(or any graph with no cycle) from online resource
///////////////////////////////////////////////////// 
struct edge{
...
void DFS(int u,vector<vector<int>>&dijkstratree){
    P[u] = ++k, rev[k] = u, L[k] = k, sdom[k] = k, DSU[k] = k;
    for(int v : dijkstratree[u]){
        if(!P[v]){
            DFS(v,dijkstratree);
            pre[P[v]] = P[u];
        }
...
void dijtree(int s, int n,vector<vector<int>>&dijkstratree,vector<vii>&graph){
    ...
    REP(i,1,n+1){
         for(auto x:graph[i]){
            int v=x.first;int w=x.second;
            if(dis[i]+w==dis[v])dijkstratree[i].pb(v);}}
}
vector<int> ans;
void path(int node){
    ans.pb(rev[node]);
...
void solve(int xyz)
{
    ...
    vector<vector<pair<int,int>>>graph(100004);
    REP(i,0,m){cin>>x>>y>>z;graph[x].pb({y,z});}
    vector<vector<int>>dijkstratree(100004);
    dijtree(1,n,dijkstratree,graph);
    DFS(1,dijkstratree);
    DOM(n);
    path(P[n]);
