...
void search(node* root, string s, vector<int>& firstpos) {
    node* parent = root;
    for (int i = 0; i < s.length(); ++i) {
        ...
        if(!outlink->visited){
            for (int ind : outlink->pattern_inds) {
                if (firstpos[ind] == -1) {
                    firstpos[ind] = i;
                }
            }
...
void solve(int xyz)
{
 ...
 node* root=new node();
  ...
  search(root,s,firstpos);
  REP(i,0,n){if(firstpos[i]!=-1){cout<<firstpos[i]-patterns[i].length()+2<<endl;}else{cout<<-1<<endl;  }}
