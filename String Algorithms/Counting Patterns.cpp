    ...
    for (int i = 0; i < patterns.size(); ++i) {
        ...
        for (int j = 0; j < patterns[i].length(); ++j) {
            ...
            if (curr->child.find(c) != curr->child.end()) {
                ...
                curr->child[c] = new_node;
                curr = new_node;
            }
        }
        curr->pattern_inds.push_back(i);
    }
}
...
void build_suffix_and_output_links(node* root) {
    ...
    while (!qu.empty()) {
        ...
        for (auto& cc : curstate->child) {
            ...
            node* temp = curstate->suffix_link;
            ...
            if (temp->child.find(cc.first) != temp->child.end()) {
                cchild->suffix_link = temp->child[cc.first];
            } else {
                cchild->suffix_link = root;
            }
 
            if (!cchild->suffix_link->pattern_inds.empty()) {
                cchild->output_link = cchild->suffix_link;
            } else {
                cchild->output_link = cchild->suffix_link->output_link;
            }
 
            qu.push(cchild);
        }
        if (!curstate->suffix_link->pattern_inds.empty()) {
...
void search(node* root, string s) {
    ...
    for (int i = 0; i < s.length(); ++i) {
        ...
        node* outlink = parent->output_link;
        while (outlink != nullptr) {
       
                    outlink->count++;
           
            outlink = outlink->output_link;
        }
    }
}
void traverse(node* root,vector<int>&firstpos){
    node* parent=root;
    for(int i:root->pattern_inds){
        firstpos[i]+=root->count;
    }
    for(auto c:root->child){
        traverse(c.second,firstpos);
    }
}
void solve(int xyz)
{
 string s;cin>>s;
 int n;cin>>n;vector<string>patterns(n);
 REP(i,0,n){cin>>patterns[i];}
 vector<int>firstpos(n,0);
 node* root=new node();
  build_trie(root,patterns);
  build_suffix_and_output_links(root);
  search(root,s);
  traverse(root,firstpos);
  REP(i,0,n){if(firstpos[i]!=-1){cout<<firstpos[i]<<endl;}else{cout<<-1<<endl;  }}
