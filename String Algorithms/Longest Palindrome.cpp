...
void solve(int xyz)
{
    ...
    vector<int>manacher(n,0);
        int l=1;int r=1;
    REP(i,0,n){
        manacher[i]=max(0ll,min(r-i,manacher[l+r-i]));
        while(i-manacher[i]>=0 && i+manacher[i]<=n && s1[i+manacher[i]]==s1[i-manacher[i]]){
            manacher[i]++;
        }
        if(i+manacher[i]>r){
            l=i-manacher[i];
    ...
    for(int i=maxindex-ans+1;i<=maxindex+ans-1;i+=2){
        cout<<s1[i];
    }
}
